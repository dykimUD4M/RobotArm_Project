...
namespace MegaFiers
{
	...
	public class MegaShapeLoft : MegaShapeBase
	{
	...
	#if UNITY_5_5 || UNITY_5_6 || UNITY_2017 || UNITY_2018 || UNITY_2019 || UNITY_2020 || UNITY_2021 || UNITY_2022 || UNITY_2023
	#else
		...
		void Start()
		{
	#if UNITY_EDITOR && !UNITY_2018_3_OR_NEWER	// && !UNITY_2019 && !UNITY_2020 && !UNITY_2021 && !UNITY_2022 && !UNITY_2023
		...
		public void BuildMeshFromLayersNew()
		{
			...
			if ( rebuild )	//&& Layers.Length > 0 )
			{
				...
				if ( Layers.Length > 0 )
				{
					...
					if ( Tangents )
						MegaShapeHelper.BuildTangents(mesh);
					...
					if ( !updating )
					{
						...
						MegaShapeLoft[]	lofts = (MegaShapeLoft[])FindObjectsByType(typeof(MegaShapeLoft), FindObjectsSortMode.None);
#else
		...
		public Matrix4x4 GetDeformMat(MegaSpline spline, float alpha, bool interp)
		{
			...
			MegaShapeMatrix.SetTR(ref wtm, ps, Quaternion.LookRotation(ps1, up));
		...
		public Matrix4x4 GetDeformMatNew(MegaSpline spline, float alpha, bool interp, float align)
		{
			...
			MegaShapeMatrix.SetTR(ref wtm, ps, rot);
			lastrot = rot;
			return wtm;
		}
		...
		public Matrix4x4 GetDeformMatNewMethod(MegaSpline spline, float alpha, bool interp, float align, ref Vector3 lastup)
		{
			...
			MegaShapeMatrix.SetTR(ref wtm, ps, rot);	//Quaternion.LookRotation(ps1, lastup));
