	...
	public class MegaLoftLayerSimple : MegaLoftLayerBase
	{
		...
		public override bool Valid()
		{
			if ( LayerEnabled && layerSection && layerPath && layerSection.splines != null && layerPath.splines != null )
			{
				...
				if ( layerPath.splines[curve].length <0.01f || layerSection.splines[crosscurve].length < 0.01f )
					return false;
		...
		Vector3 GetCross(MegaShapeLoft loft, float ca)
		{
			...
			MegaShapeMatrix.Rotate(ref tm1, new Vector3(Mathf.Deg2Rad * crossRot.x, Mathf.Deg2Rad * crossRot.y, Mathf.Deg2Rad * crossRot.z));
		...
		public override bool PrepareLoft(MegaShapeLoft loft, int sc)
		{
			...
			crosssize = MegaShapeHelper.Extents(crossverts, out crossmin, out crossmax);
