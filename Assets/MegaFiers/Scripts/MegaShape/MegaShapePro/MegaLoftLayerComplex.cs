	...
	public class MegaLoftLayerComplex : MegaLoftLayerSimple
	{
		...
		public override bool Valid()
		{
			if ( LayerEnabled && sections != null && sections.Count > 0 )
			{
				for ( int i = 0; i < sections.Count; i++ )
				{
					if ( sections[i].shape == null || sections[i].shape.splines[sections[i].curve].length < 0.01f )
						return false;
				}

				if ( layerPath.splines[curve].length < 0.01f )
					return false;
		...
		Vector3 GetCross(int csect, float ca, Vector3 off)
		{
			...
			MegaShapeMatrix.Translate(ref tm1, pivot);
			Vector3 rot = crossRot + lsection.rot;
			MegaShapeMatrix.Rotate(ref tm1, new Vector3(Mathf.Deg2Rad * rot.x, Mathf.Deg2Rad * rot.y, Mathf.Deg2Rad * rot.z));
		...
		void BuildPolyShape(MegaLoftSection lsection, int steps, Vector3 off, float width)
		{
			...
			lsection.crosssize = MegaShapeHelper.Extents(lsection.crossverts, out lsection.crossmin, out lsection.crossmax);
		}
		...
		public Matrix4x4 GetDeformMat(MegaSpline spline, float alpha, bool interp)
		{
			...
			MegaShapeMatrix.SetTR(ref wtm1, ps, Quaternion.LookRotation(ps1, locup));
		...
		public Matrix4x4 GetDeformMatNewMethod(MegaSpline spline, float alpha, bool interp, ref Vector3 lastup)
		{
			...
			//wtm1.SetTRS(ps, Quaternion.LookRotation(ps1, locup), Vector3.one);
			MegaShapeMatrix.SetTR(ref wtm1, ps, Quaternion.LookRotation(ps1, lastup));	//locup));
