...
namespace MegaFiers
{
	...
	public class MegaSpline
	{
		...
		public bool ContainsSubDiv(Vector3 p, int sub = 4)
		{
			if ( sub == 0 )
				return Contains(p);
			...
			for ( int i = 0; i < knots.Count; i++ )
			{
				...
				for ( int s = 0; s < sub; s++ )
				{
					Vector3 kp1 = knots[i].Interpolate((float)s / (float)sub, knots[j]);
					Vector3 kp2 = knots[i].Interpolate((float)(s + 1) / (float)sub, knots[j]);
		...
		public Vector3 Interpolate(float alpha, bool type, ref int k)
		{
			...
			else
			{
				...
				{
					//Debug.Log("knots " + knots.Count + " seg " + seg);
	...
	public class MegaShape : MonoBehaviour
	{
		...
		public Matrix4x4 GetMatrix()
		{
			...
			switch ( axis )
			{
				case MegaAxis.X:	MegaShapeMatrix.RotateY(ref tm, -Mathf.PI * 0.5f);
					MegaShapeMatrix.Scale(ref tm, -Vector3.one, false);
					break;
				//case MegaAxis.X: MegaMatrix.RotateY(ref tm, Mathf.PI * 0.5f); break;
				case MegaAxis.Y: MegaShapeMatrix.RotateX(ref tm, Mathf.PI * 0.5f); break;
		...
		public void AutoCurve()
		{
			if ( autosmooth )
			{
				if ( knots.Count > 1 )
				{
					if ( smoothMode == SmoothMode.Adjustable )
						ComputeControlPointsOld();
					else
						ComputeControlPointsNew();
				}
			}
			else
				CalcLength();
		...
		public void ComputeControlPointsNew(MegaSpline spline)
		{
			...
			//spline.knots[n].invec = iv;
			//spline.knots[n].outvec = ov;
		...
		Matrix4x4 GetDeformMat(float percent)
		{
			...
			MegaShapeMatrix.SetTR(ref wtm, ps, rotation * twistrot);
		...
		public void BuildBoxCrossSection(float width, float height)
		{
			...
			MegaShapeMatrix.RotateY(ref mat, sang);	//rotate);
		...
		void BuildRibbonMesh()
		{
			...
			switch ( axis )
			{
				case MegaAxis.X: MegaShapeMatrix.RotateY(ref tm, -Mathf.PI * 0.5f); break;
				case MegaAxis.Y: MegaShapeMatrix.RotateX(ref tm, -Mathf.PI * 0.5f); break;
				case MegaAxis.Z: break;
			}

			MegaShapeMatrix.SetTrans(ref tm, Pivot);
			...
			shapemesh.RecalculateNormals();
			if ( CalcTangents )
				MegaShapeHelper.BuildTangents(shapemesh);
		}
